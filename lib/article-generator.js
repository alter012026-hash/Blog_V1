...
// ─── Prompt ───────────────────────────────────────────────────────────
function buildPrompt({ topic, generation }, retryHint) {
 ...
 */
async function generateValidatedArticle({ topic, category, generation, existingSignatures = [], maxRetries = 2 }) {
  let retryHint = null;
  let lastAttempt = null;

  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const prompt = buildPrompt({ topic, generation }, retryHint);
