 ...
 *
 * Usa a MESMA ordem de providers da geração de posts (Grok → Groq →
 * OpenRouter → Gemini, a ordem padrão de generateWithFallback). Atenção:
 * isso inclui o Groq, que antes era reservado exclusivamente para os
 * posts (ver lib/quiz-generator.js, que usa só OpenRouter → Gemini pelo
 * motivo oposto). Com essa mudança, perguntas de leitores em produção
 * PODEM competir pela cota do Groq com a geração diária de artigos via
 * GitHub Actions. Se a geração de posts começar a falhar por falta de
 * cota do Groq, esse é o primeiro lugar a revisar.
 *
...
const QA_PROVIDER_ORDER = ["Grok", "Groq", "OpenRouter", "Gemini"];
...
async function answerArticleQuestion({ title, category, content, question }) {
  try {
    ...
    // Loga o motivo real (ex: "Todos os provedores falharam — Grok: GROK_API_KEY
    // ausente | OpenRouter: ... | Gemini: ...") nos logs da função na Vercel.
    // Sem isso, uma falha de configuração (chave ausente/inválida) fica
    // indistinguível de uma instabilidade passageira dos providers.
    console.error("[article-qa] falha ao gerar resposta:", err.message);
    return null;
  }
}
