 ...
 *
 * Reaproveita `generateWithFallback` de lib/article-generator.js, usando
 * Grok (xAI) como provider principal, com OpenRouter e Gemini como
 * fallback. Grok é um provider PRÓPRIO, diferente do Groq — não compete
 * com a cota do Groq, que continua 100% reservada para a geração diária
 * de posts via GitHub Actions. Nenhuma variável de ambiente nova é
 * necessária além da GROK_API_KEY já usada pelo Grok no resto do
 * projeto (ver callGrok em lib/article-generator.js).
 *
...
const QA_PROVIDER_ORDER = ["Grok", "OpenRouter", "Gemini"];
