...
async function addSubscriber({ email, firstName }) {
  ...
  try {
    const data = await resendRequest(`/audiences/${audienceId}/contacts`, {
      method: "POST",
      body: JSON.stringify({
...
/** Remove (descadastra) um contato do audience. */
...
/** Lista os Contacts do audience (usado pelo admin para mostrar o total). */
async function listSubscribers() {
  const audienceId = await getAudienceId();
  const data = await resendRequest(`/audiences/${audienceId}/contacts`, { method: "GET" });
...
/**
 * Busca o audience_id da conta — a API do Resend (a partir de 2025) exige
 * que o broadcast seja enviado para um audience_id específico.
 * Tenta: 1) variável RESEND_AUDIENCE_ID do env, 2) busca o primeiro
 * audience disponível via GET /audiences.
 */
async function getAudienceId() {
  // Prioridade: variável de ambiente explícita
  const envId = process.env.RESEND_AUDIENCE_ID?.trim();
  if (envId) return envId;

  // Fallback: busca a lista de audiences e pega o primeiro
  try {
    const data = await resendRequest("/audiences", { method: "GET" });
    const audiences = data?.data || [];
    if (audiences.length > 0) {
      return audiences[0].id;
    }
  } catch (err) {
    console.error("Erro ao buscar audiences do Resend:", err.message);
  }

  throw new Error(
    "Nenhum audience_id encontrado. Configure RESEND_AUDIENCE_ID nas variáveis de ambiente " +
    "com o ID do seu Audience no painel do Resend (Contacts → ícone de engrenagem)."
  );
}
 ...
 *
 * NOTA: A partir de meados de 2025 o Resend exige audience_id no broadcast.
 * Configure RESEND_AUDIENCE_ID no Vercel/GitHub Actions com o ID do seu
 * Audience (visível em Contacts → engrenagem no painel do Resend).
 */
async function sendDailyBroadcast({ posts, siteUrl, siteName }) {
  ...
  // A API do Resend exige audience_id a partir de 2025
  const audienceId = await getAudienceId();
